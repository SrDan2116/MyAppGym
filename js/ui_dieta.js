...
export function initDieta(database) {
  ...
  const hoy = new Date();
  fechaSeleccionada = `${hoy.getFullYear()}-${(hoy.getMonth() + 1)
    .toString()
...
function agregarSnack() {
  const snacks = comidasDelDia.filter((c) =>
    c.nombre.startsWith("Snack")
  ).length;
  comidasDelDia.push({
...
function moverComida(index, direccion) {
  ...
  [comidasDelDia[index], comidasDelDia[nuevoIndex]] = [
    comidasDelDia[nuevoIndex],
    comidasDelDia[index],
  ];
  renderizarComidas();
}
...
function renderizarComidas() {
    ...
    div.innerHTML = `
            <div class="comida-header">
                ...
                <div class="comida-controles">
                    ...
                    ${
                      index > 2
                        ? `<button class="btn-borrar-comida" data-index="${index}" style="background:var(--color-error)">X</button>`
                        : ""
                    }
                </div>
            </div>
            <div class="comida-detalles">
                ...
                <p>🔥 ${comida.macros.calorias} kcal | 🥩 P: ${
      comida.macros.proteinas
    } | 🍞 C: ${comida.macros.carbohidratos} | 🥑 G: ${comida.macros.grasas}</p>
            </div>
        `;
  ...
  document
    .querySelectorAll(".btn-mover-arriba")
    .forEach((b) =>
      b.addEventListener("click", (e) =>
        moverComida(parseInt(e.target.dataset.index), -1)
      )
    );
  document
    .querySelectorAll(".btn-mover-abajo")
    .forEach((b) =>
      b.addEventListener("click", (e) =>
        moverComida(parseInt(e.target.dataset.index), 1)
      )
  ...
  document
    .querySelectorAll(".btn-editar-comida")
    .forEach((b) =>
      b.addEventListener("click", (e) =>
        abrirModal(parseInt(e.target.dataset.index))
      )
    );
  document
    .querySelectorAll(".btn-borrar-comida")
    .forEach((b) =>
      b.addEventListener("click", (e) =>
        eliminarComida(parseInt(e.target.dataset.index))
      )
    );
}
