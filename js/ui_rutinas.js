...
const txtRangoReps = document.getElementById('txt-rango-reps'); 
...
export function initRutinas(database) {
    ...
    btnAgregarEjercicioADia.addEventListener('click', () => {
        ...
        const rangoReps = txtRangoReps.value; 

        if (nombreEjercicio.trim() === '' || isNaN(series) || series <= 0) { 
            alert('Ingresa nombre y series válidos.'); return; 
        }

        ejerciciosTemporalesParaDia.push({ 
            nombre: nombreEjercicio, 
            series: series, 
            targetReps: rangoReps 
        });

        renderizarEjerciciosTemporalesParaDia();
        txtNombreEjercicio.value = ''; numSeries.value = ''; txtRangoReps.value = ''; 
    ...
    btnGuardarDiaEnRutina.addEventListener('click', () => {
        ...
        txtNombreDia.value = ''; ejerciciosTemporalesParaDia = []; 
        renderizarEjerciciosTemporalesParaDia();
    });
    ...
    btnGuardarRutinaCompleta.addEventListener('click', () => {
        ...
        if (rutinaTemporal.nombre.trim() === '' || rutinaTemporal.dias.length === 0) { 
            alert('Ingresa un nombre de rutina y guarda al menos un día.'); return; 
        }
        ...
        if (idRutinaEditando !== null) {
            rutinaTemporal.id = idRutinaEditando; 
            request = store.put(rutinaTemporal);
        } else {
            delete rutinaTemporal.id; 
...
function cargarRutinasGuardadas() {
    ...
    const store = transaction.objectStore('rutinas');
    const request = store.openCursor(); // Usamos cursor simple, sin filtros
    ...
    request.onsuccess = (event) => {
        ...
        if (cursor) {
            ...
            divBody.className = 'rutina-detalles oculto';
            
            let contenidoDetalles = '';
            rutina.dias.forEach(dia => {
                contenidoDetalles += `<p><strong>${dia.nombreDia}</strong></p><ul>`;
                dia.ejercicios.forEach(ej => { 
                    const metaInfo = ej.targetReps ? ` <span style="color:var(--color-secundario)">| Meta: ${ej.targetReps}</span>` : '';
                    contenidoDetalles += `<li>${ej.nombre} (${ej.series} series${metaInfo})</li>`; 
                });
...
function renderizarEjerciciosTemporalesParaDia() {
    ...
    ejerciciosTemporalesParaDia.forEach((ej, index) => {
        ...
        const metaTexto = ej.targetReps ? ` | Meta: ${ej.targetReps}` : '';
        div.innerHTML = `<span>${ej.nombre} (${ej.series} series${metaTexto})</span> <button type="button" class="btn-borrar-item" data-index="${index}">&times;</button>`;
...
function borrarRutina(id) {
    ...
    deleteRequest.onsuccess = () => { cargarRutinasGuardadas(); };
